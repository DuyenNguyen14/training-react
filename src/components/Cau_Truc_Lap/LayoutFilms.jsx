...
export default class LayoutFilms extends Component {
    renderFilmCard = () => {
        return data.map((film, index) => {
            return <div className="col-2 mt-3" key={index}>
                <div className="card">
                    <img style={{ height: "210px" }} src={film.hinhAnh} alt="..." />
                    <div className="card-body">
                        <h4 style={{ height: '50px' }}>{film.tenPhim.length > 20 ? film.tenPhim.slice(0, 20) + '...' : film.tenPhim}</h4>
                        <p style={{ height: '65px' }}>{film.moTa.length > 65 ? film.moTa.slice(0, 65) + "..." : film.moTa}</p>
                    </div>
                </div>
            </div>
        })
