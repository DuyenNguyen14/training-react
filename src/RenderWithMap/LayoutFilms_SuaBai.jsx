...
export default class LayoutFilms_SuaBai extends Component {

    renderFilmCard = () => {
        return data.map((film, index) => {
            return <div className="col-lg-2 col-md-3 mt-4" key={index}>
                <div className="card pb-3">
                    <img src={film.hinhAnh} alt="..." height={350} />
                    <div className="card-body">
                        <h4 style={{ height: 55 }}>{film.tenPhim.length > 20 ? film.tenPhim.slice(0, 20) + '...' : film.tenPhim}</h4>
                        <p style={{ height: 85 }}>{film.moTa.length > 100 ? film.moTa.slice(0, 100) + '...' : film.moTa}</p>
                    </div>
                </div>
            </div>
        })
        ...
        return (
            <div className='bg-films'>
                <div className="h-100" style={{ background: 'rgba(0,0,0,.5)', minHeight: '100vh' }}>
                    <nav className="navbar navbar-expand-sm navbar-dark px-5" style={{ background: 'rgba(0,0,0,.6)' }}>
